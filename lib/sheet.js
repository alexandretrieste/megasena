...
async function getAuth() {
  ...
  try {
    ...
    // Create JWT client with explicit key format handling
    ...
    // Force authentication to catch errors early
    await auth.authorize();
    
    return auth;
  } catch (error) {
    console.error('Erro ao criar autenticação:', error);
    // If it's an OpenSSL error, provide more helpful message
    if (error.code === 'ERR_OSSL_UNSUPPORTED' || error.message.includes('DECODER')) {
      throw new Error('Erro ao processar chave privada. Verifique se a chave está no formato correto (com quebras de linha \\n).');
    }
